...
const setup = () => {
    const logEndPoint = (req, res, next) => {
        console.log("You have hit the [POST] api/songs/id/rating endpoint");
        next();
    };
    ...
    const updateSongLike = (req, res) => {
        ...
        */
        ...
        Song
            .findByPk(id) // Find key by "id" and returns value as "id_value"
            .then(id_value => {

                // Check "rate" value and update song's rating
                if (rate == 1)
                    id_value.increment("rating", {by: 1});
                else
                    id_value.decrement("rating", {by: 1});
